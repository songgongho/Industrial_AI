...
* **라이브러리 (Libraries)**: **OpenCV**, Pandas, NumPy, Scikit-learn, TensorFlow, Keras, Matplotlib, Seaborn, Flask
...
* **주요 내용**:
    ...
    * 보조비(3만원) 기준 부족분 발생 시 1,000원 단위 올림 스마트 공동구매 분배 알고리즘 적용
    * 공동구매 내역 확인 및 금액 직접 수정이 가능한 인터랙티브 UI 구현
    * 클릭 및 복사/붙여넣기(Ctrl+V)를 활용한 직관적인 영수증/활동 사진 첨부 기능
    * 프린트 전용 CSS(@media print)를 적용한 A4 규격 인쇄 레이아웃 최적화

* **결과 요약**:
    수기 계산으로 인한 정산 오류를 원천 차단하고 사내 동호회비 증빙 서류 작성 시간을 대폭 단축하여 업무 효율성을 증대시켰습니다
...
### 10. 👁️ OpenCV 기반 영상 처리 심화 및 실습 (2026.03 Update)
...
* **[➡️ 최종 결과 보고서]([https://github.com/songgongho/Industrial_AI/blob/main/Portfolio-OpenCV/docs/OPENCV_REPORT.md](https://github.com/songgongho/Industrial_AI/tree/main/Portfolio-OpenCV/results))**
...
* ✅ 결측치 처리 및 특성 공학 (Feature Engineering)

### 머신러닝 & 딥러닝
* ✅ 지도/비지도 학습 및 이상 탐지 (IF, LOF, Autoencoder)
* ✅ 신경망 아키텍처 설계 (CNN, RNN, LSTM) 및 하이퍼파라미터 최적화
...
### 🟡 중급자 (Intermediate)
1. 전력 수요 예측 (시계열 기초)
2. **OpenCV 심화 실습 (컴퓨터 비전 기초)**
3. 이상 탐지 #1 (알고리즘 비교)

### 🔴 고급자 (Advanced)
1. 버스 이상 탐지 (실무 프로젝트)
2. 동호회 정산 자동 생성기 (업무 자동화 응용)
